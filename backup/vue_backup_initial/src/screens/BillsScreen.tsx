...
import { Inbox, CheckCircle, Clock, AlertCircle, Camera } from 'lucide-react-native';
...
export const BillsScreen: React.FC = () => {
  ...
  const { bills, markBillAsPaid, runClarityEngine, approvePendingBill } = useStore();
  ...
  const handleBillPress = useCallback((bill: Bill) => {
    if (bill.status === 'pending_approval' || bill.status === 'paused') {
      setPendingApprovalBill(bill);
      setShowBiometric(true);
    } else {
      handleQuickPay(bill);
      ...
      <View style={styles.header}>
        <View style={styles.headerTop}>
          ...
          <TouchableOpacity 
            ...
            onPress={() => navigation.navigate('BillScanResult' as never)}
          >
      ...
      <FlatList
        ...
        renderItem={({ item: bill }) => (
          <BillCard
            ...
            onQuickPay={() => handleBillPress(bill)}
